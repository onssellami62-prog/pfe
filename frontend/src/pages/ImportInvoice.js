    ...
    return (
        <div className="import-container animate-slideUp">
            ...
            <div className="mb-10 text-center">
                <span className="bg-blue-100 text-blue-700 text-[10px] font-black uppercase px-3 py-1 rounded-full tracking-widest mb-4 inline-block">Moteur OCR v3.0 Actif</span>
            ...
            <div className="main-dropzone border-4 border-dashed border-blue-50 bg-blue-50/20 rounded-[30px] p-16 flex flex-col items-center justify-center hover:border-blue-200 transition-all cursor-pointer group" onClick={handleChooseFile}>
                <div className="w-20 h-20 bg-blue-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-blue-200 group-hover:scale-110 transition-transform mb-6">
                ...
                <button className="mt-8 px-10 py-4 bg-gray-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-gray-800 transition-all shadow-xl shadow-gray-200">+ Importer le document</button>
            </div>
                    ...
                    {files.map((file, idx) => (
                        <div key={idx} className="bg-white border-2 border-gray-100 rounded-3xl p-6 flex items-center gap-6 animate-fadeIn shadow-sm hover:shadow-md transition-shadow">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${file.status === 'error' ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400'}`}>
                                {file.status === 'ready' ? '🏢' : (file.status === 'extracting' ? '📄' : (file.status === 'ocr' ? '⚡' : '📂'))}
                            </div>
                            <div className="flex-1">
                                ...
                                <div className="h-2 bg-gray-50 rounded-full overflow-hidden">
                                    <div className={`h-full transition-all duration-500 ${file.status === 'ready' ? 'bg-green-500' : 'bg-blue-600'}`} style={{ width: `${file.progress}%` }}></div>
                                </div>
                                <div className="flex items-center gap-3 mt-2">
                                    <span className={`text-[9px] font-black uppercase tracking-widest ${file.status === 'ready' ? 'text-green-600' : 'text-blue-500'}`}>
                                        {file.status === 'ocr' ? 'Traitement IA OCR en cours...' : (file.status === 'ready' ? 'Données Certifiées - XML Disponible' : 'Lecture Structurelle...')}
                                    </span>
                                </div>
                            </div>
                            {file.status === 'ready' && (
                                <button onClick={() => handleOpenPreview(file)} className="bg-blue-600 text-white font-black text-[10px] px-6 py-3 rounded-xl hover:bg-blue-700 uppercase tracking-widest shadow-lg shadow-blue-200">Aperçu XML</button>
                            )}
                        </div>
                    ))}
                </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-slideUp">
                        ...
                        <div className="p-6 border-t border-gray-100 flex gap-4 bg-gray-50">
                            <button onClick={copyXml} className="flex-1 bg-white border-2 border-gray-100 text-gray-700 font-black text-[10px] py-3 rounded-xl hover:bg-gray-100 uppercase tracking-widest">📋 Copier le code</button>
                            <button onClick={handleDownloadXml} className="flex-1 bg-blue-600 text-white font-black text-[10px] py-3 rounded-xl hover:bg-blue-700 uppercase tracking-widest shadow-lg shadow-blue-200">📥 Télécharger .xml</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="mt-12 pb-12">
                ...
                <button className={`w-full py-6 rounded-3xl text-white font-black uppercase tracking-widest shadow-2xl transition-all ${files.length === 0 || isProcessing ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-900 hover:scale-[1.01] shadow-gray-300'}`} disabled={files.length === 0 || isProcessing} onClick={() => files.filter(f => f.status === 'ready').forEach(f => downloadXml(generateTeifXml(f.extraction.issuer, f.extraction.invoice), `TEIF_${f.extraction.issuer.name}_${f.extraction.invoice.number}.xml`))}>
